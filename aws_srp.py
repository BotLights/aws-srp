...
def add_emf_header(request, **kwargs):
    ...
    for x in headers:
      request.headers.add_header(x, headers[x])
    # print("HEADER")
...
class AWSSRP(object):
    ...
    def __init__(self, username, password, pool_id, client_id, pool_region=None,
        ...
        # print("SETUP")
        self.client = client if client else boto3.client('cognito-idp', region_name = pool_region, config = config)
        ...
        event_system.register_first('before-sign.*.*', add_emf_header)
